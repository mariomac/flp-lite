...
func Protobuf() node.MiddleFunc[[]byte, locked.Var[map[string]interface{}]] {
	log.Debug("creating protobuf decoder")
	return func(in <-chan []byte, out chan<- locked.Var[map[string]interface{}]) {
		...
		for pbRaw := range in {
			...
			out <- locked.For(pbFlowToMap(&record))
		}
